...
# Modified date : 2019-01-12 15:22
# Author : DARREN
# Describe : not set
...
#        dic["reg_lambda"] = 0.05
    ...
    def _print_test_status(self, model):
        ...
        best_str = "best_epoch:%s best_step:%s best_test_acc:%s dual_time:%s" % (model["best_epoch"], model["best_step"], model["best_test_accuracy"],model["dual_time"])
    ...
    def _test_update_model(self, model, avg_loss, accuracy):
        if accuracy > model["best_test_accuracy"]:
            ...
            model["dual_time"] = time.time() - self.start_time
